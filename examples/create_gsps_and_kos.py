...
if __name__ == "__main__":
    ...
    gsps.set_dicom_attribute("SoftcopyVOILUTSequence", [{
        "WindowCenter": "-600",
        "WindowWidth": "1500"
    }])
    ...
    gsps.add_graphical_layer("SCROLLBAR", 3, recommended_cielab_value=WHITE.dcm_format())
    gsps.add_graphic_object(dcm_files[0], "LAYER1", [50, 50, 50, 100, 100, 100, 100, 50, 50, 50], "POLYLINE")
    gsps.add_graphic_object(dcm_files[0], "LAYER1", [256, 256], "POINT")
    gsps.add_graphic_object(dcm_files[0], "LAYER1", [200, 200, 300, 300, 400, 400, 500, 500], "POLYLINE",
                            cielab_value=PURPLE.dcm_format(), 
                            shadow_style="NORMAL",
                            line_thickness=5.0)                   
    gsps.add_graphic_object(dcm_files[0], "LAYER2", [70, 70, 70, 170, 170, 170, 120, 120, 170, 70, 70, 70], "POLYLINE",
                            graphic_filled="Y")
    ...
    for ind, dcm_file in enumerate(sorted_dcm_files):
        gsps.add_graphic_object(dcm_file, "SCROLLBAR", [505, 50, 507, 50, 507, 461, 505, 461, 505, 50], "POLYLINE")
        gsps.add_graphic_object(dcm_file, "SCROLLBAR",
                                [497, int(50 + (461.0 - 50.0) / float(len(dcm_files) - 1) * (len(dcm_files) - ind_finding - 1)), 
                                501, int(50 + (461.0 - 50.0) / float(len(dcm_files) - 1) * (len(dcm_files) - ind_finding - 1))],
                                "POLYLINE",
                                cielab_value=ORANGE.dcm_format(),
                                line_thickness=3.0,
                                shadow_style="OFF")
        gsps.add_graphic_object(dcm_file, "SCROLLBAR",
                                [503, int(50 + (461.0 - 50.0) / float(len(sorted_dcm_files) - 1) * (len(sorted_dcm_files) - ind - 1)), 
                                509, int(50 + (461.0 - 50.0) / float(len(sorted_dcm_files) - 1) * (len(sorted_dcm_files) - ind - 1))],
                                "POLYLINE")
