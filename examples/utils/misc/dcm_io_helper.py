...
def read_and_sort_dicom_files(dcm_files, return_dcm_files=False):
    ...
    dcm_handlers = [pydicom.dcmread(item, force=True) for item in dcm_files]
...
def construct_T_R_S_from_dcm_handlers(dcm_handlers):
    ...
    z_dir = np.array(dcm_handlers[1].ImagePositionPatient) - np.array(
        dcm_handlers[0].ImagePositionPatient
    )
    ...
    r = np.concatenate(
        (np.expand_dims(x_dir, 1), np.expand_dims(y_dir, 1), np.expand_dims(z_dir, 1)),
        axis=1,
    )
